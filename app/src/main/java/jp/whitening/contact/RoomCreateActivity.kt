...
class RoomCreateActivity : AppCompatActivity(), View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == CHOOSER_REQUEST_CODE) {
            ...
            roomIcon.setImageBitmap(resizedImage)
    ...
    override fun onClick(v: View) {
        if (v === roomIcon) {
            ...
            val roomRef = dataBaseReference.child(ContentsPATH).child(RoomsPATH)
            ...
            // タイトルと本文を取得する
            val theme = themeText.text.toString()
            val detail = detailText.text.toString()

            if (theme.isEmpty()) {
                // タイトルが入力されていない時はエラーを表示するだけ
                Snackbar.make(v, getString(R.string.input_theme), Snackbar.LENGTH_LONG).show()
                return
            }

            if (detail.isEmpty()) {
                // 質問が入力されていない時はエラーを表示するだけ
                Snackbar.make(v, getString(R.string.room_message), Snackbar.LENGTH_LONG).show()
                return
            }
            ...
            // 添付画像を取得する
            val drawable = roomIcon.drawable as? BitmapDrawable
            ...
            if (drawable != null) {
                ...
                data["roomIcon"] = bitmapString
            } else {
                Snackbar.make(v, "画像を設定してください", Snackbar.LENGTH_LONG).show()
                return
            }

            roomRef.push().setValue(data, this)
